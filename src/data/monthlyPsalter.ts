/**
 * El Salterio distribuido en 30 días (LOC 1928 / BCP).
 * Los 150 Salmos repartidos por día del mes, Oración Matutina y Vespertina.
 * Extraído del DOCX integrado del LOC. En meses de 31 días, el día 30 se repite el 31.
 */
export interface MonthlyPsalterDay { morning: number[]; evening: number[]; }

export const monthlyPsalter: Record<number, MonthlyPsalterDay> = {
  ...
  24: { morning: [116, 117, 118], evening: [119] },
  25: { morning: [119], evening: [119] },
  26: { morning: [119], evening: [119] },
...
/** Salmos del Salterio Mensual para una fecha y oficio. El día 31 usa el reparto del 30. */
export function getMonthlyPsalms(date: Date, office: "morning" | "evening"): number[] {
  const dom = Math.min(date.getDate(), 30);
  const entry = monthlyPsalter[dom];
  return entry ? entry[office] : [];
}
